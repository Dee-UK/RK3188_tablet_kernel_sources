...
#define LSM303D_DEVID			(0x33)	//chip id 0x49
...
static int __init gsensor_lsm303d_init(void)
{
...
static void __exit gsensor_lsm303d_exit(void)
{
	struct sensor_operate *ops = gsensor_get_ops();
	int type = ops->type;
...
module_init(gsensor_lsm303d_init);
module_exit(gsensor_lsm303d_exit);
